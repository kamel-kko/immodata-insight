...
(function () {
  ...
  function init() {
    ...
    const scraperCards = self.__immodata.currentCards || [];
    log.info('currentCards du scraper : ' + scraperCards.length);

    // Strategie 1 : utiliser les cards du scraper (ont deja element + donnees)
    let attached = 0;
    scraperCards.forEach((cardData) => {
      if (cardData.element) {
        attachToCard(cardData.element, cardData);
        attached++;
      }
    });
    log.info('Cards attachees via scraper : ' + attached);

    // Strategie 2 : si le scraper n'a rien trouve, chercher par selecteurs CSS
    if (attached === 0) {
